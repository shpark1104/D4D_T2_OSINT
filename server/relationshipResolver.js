...
const TRACKED_IOC_TYPES = new Set(["email", "btc_address", "eth_address", "telegram", "domain", "url"]);
const ALIAS_KEYS = /(alias|user(name)?|nick(name)?|handle|author|actor|channel|screen_?name|display_?name|name)$/i;
const GENERIC_ALIASES = new Set(["unknown", "admin", "user", "none", "null", "true", "false", "telegram", "channel", "message"]);
...
function relationScore(relation, left, right, base = 0.5) {
  ...
  for (const entry of aliasOccurrences.values()) {
    ...
    addRelationship(
      relationships,
      {
        ...
        evidence: [`repeated in ${entry.docs.size} result documents`, ...entry.contexts.slice(0, 3).map((context) => `context: ${context}`)],
        sources: Array.from(entry.docs),
        identifiers: ["alias"]
      },
      reviewOverrides
